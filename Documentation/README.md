...
**Inter-service auth:** Downstream services trust `X-User-Id` / `X-User-Roles` headers forwarded by the gateway, and independently verify the JWT signature as defense-in-depth (shared HS256 secret via `JWT_SECRET` env var).

**Inter-service calls:** Synchronous REST via Spring WebClient. `validation-service` calls `escalation-service` when a customer response is "unresolved".
...
| **AGENT** | `agent@insightloop.com` | `Agent@123` | View dashboard, create/edit commitments and support logs *(Delete is disabled)*. |
...
11. **/pitch** — in-app 6-slide product deck.

---

## [API Reference](./api.md)

All requests go through the gateway at `http://localhost:8080`. Attach `Authorization: Bearer <token>` after login.

### Auth (required by assessment)

| Method | Endpoint | Auth | Roles | Purpose |
|---|---|---|---|---|
| POST | `/api/auth/login` | public | — | Returns `{ accessToken, user }` |
| POST | `/api/auth/register` | public | — | Creates a USER (self-service) |
...
| POST | `/api/roles/assign` | required | ADMIN | `{ userId, role }` |
| GET | `/api/me/permissions` | required | any | Current user's roles + permissions |

### InsightLoop domain

| Method | Endpoint | Roles | Purpose |
|---|---|---|---|
| GET | `/api/interactions` | any | List interactions |
| POST | `/api/interactions` | AGENT, ADMIN | Create interaction |
| POST | `/api/interactions/{id}/feedback` | any | Capture CSAT + comment |
| GET | `/api/commitments` | any | List (filter by status) |
| POST | `/api/commitments` | AGENT, ADMIN | Create commitment |
| GET | `/api/commitments/{id}` | any | Detail with audit trail |
| POST | `/api/validations/{commitmentId}/respond` | any | `{ resolved: bool, comment }` — triggers escalation if false |
| GET | `/api/escalations` | AGENT, ADMIN | Queue |
| PATCH | `/api/escalations/{id}` | AGENT, ADMIN | Update status/assignee |
| GET | `/api/analytics/summary` | any | Dashboard KPIs |
...
curl -X POST http://localhost:8080/api/auth/login \
  ...
  -d '{"email":"admin@insightloop.com","password":"Admin@123"}'
```
...
```json
{
  ...
  "user": {
    ...
    "roles": ["ADMIN"],
    "permissions": ["user:read","user:write","commitment:write","escalation:manage"]
  }
}
```
