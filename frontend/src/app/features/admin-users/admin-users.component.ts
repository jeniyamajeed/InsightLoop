                ...
                <ng-container *ngIf="editingId !== u.id; else editRow">
                  ...
                  <td class="text-right">
                    <div class="inline-flex items-center justify-end gap-2">
                      <button *ngIf="u.id !== auth.user()?.id" (click)="startEdit(u)" title="Edit" class="p-1.5 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-all duration-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                      </button>
                      <button *ngIf="u.id !== auth.user()?.id" (click)="remove(u)" title="Delete" class="p-1.5 rounded-lg text-rose-600 hover:bg-rose-50 transition-all duration-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                      </button>
                      <span *ngIf="u.id === auth.user()?.id" class="text-xs text-slate-400 italic px-2">Current User</span>
                    </div>
                  </td>
                </ng-container>
